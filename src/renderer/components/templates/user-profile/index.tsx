  ...
  return (
    <main>
      ...
      <section tw="space-y-4 -mt-16 p-4">
          ...
          <section tw="flex flex-col self-end flex-grow space-y-1">
            ...
            <section tw="flex flex-row flex-grow justify-between">
              ...
              <section tw="truncate space-y-1">
                {(props.user && <h1 tw="text-2xl">{props.user?.name}</h1>) || <SkeletonLoader tw="h-8 w-60" />}
                {/* {(props.user && (
                  <strong tw="text-lg text-gray-700 dark:(text-gray-300)">{props.user?.community_role}</strong>
              ...
              <section tw="flex flex-col space-y-1 space-x-0 md:(flex-row space-y-0 space-x-1)">
                <Button tw="h-8" magnitude="small">
                  <FormattedMessage {...UserMessages['generic.user.messageAction']} />
                </Button>
                <Button tw="h-8" color="blue" magnitude="small">
                  <FormattedMessage {...UserMessages['generic.user.followAction']} />
                </Button>
              </section>
            </section>
            ...
            <section tw="flex flex-row items-center space-x-1">
                ...
                <ModeBadges
                  modes={
                    Object.entries(props.user.preferences)
                      ...
                      .map(([k]) => k) as Array<Definitions['Journey.Beatmap']['mode']>
                  }
                />
              ...
              {/* <span tw="flex flex-row text-xs text-gray-700 space-x-2">
                <span tw="font-bold text-gray-700 dark:text-gray-300 sm:(hidden) md:(hidden) lg:(hidden) xl:(block)">
                  {(props.user && (
                    <FormattedMessage id="templates.user.profile.availability" defaultMessage="Available for" />
                  )) || <SkeletonLoader tw="h-4 w-24" />}
                </span>
                {(props.user?.availability && <UserAvailability {...props.user.availability} />) || (
        ...
        <article>
          {props.user !== null ? (
            <Markdown>{props.user.description || 'User has no description (っ °Д °;)っ'}</Markdown>
          ) : (
        ...
        {/* {props.user !== null && open.length === 0 && closed.length === 0 && pending.length === 0 && (
          <p>
            <FormattedMessage
